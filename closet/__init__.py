...
class RiakModel(micromodels.Model):
    ...
    def __getattr__(self, name):
        if super(RiakModel, self)._lazy:
            self.__init__(_key=self.key, **self._lazy.get_data())
    ...
    @classmethod
    def get(cls, key):
        return cls(_key=key, lazy=cls.bucket.get(key))
    ...
    def store(self):
        self.bucket.new(self._key, self.to_dict(serial=True)).store()